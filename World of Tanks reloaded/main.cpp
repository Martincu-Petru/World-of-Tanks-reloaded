...
Font textStyle;
Text optiunea[4], optiuneaExit[3], optiuneaResume[3], optiuneaJoc[2], optiuneHowToPlay[2];
...
void howToPlay()
{
	...
	/*optiuneHowToPlay[2].setFont(textStyle);
	optiuneHowToPlay[2].setFillColor(Color::Red);
	optiuneHowToPlay[2].setString("Next");
	optiuneHowToPlay[2].setPosition(Vector2f(1366 / 3 + 600, 768 - 100));*/
...
int main()
{
	...
	Text healthLevel, SpeedTank, howToPlay;
	Font fontHealth, fontHTP;
		...
		if (opt == 1) {
			...
			if (opt == 3)
			{
				...
				if (mainTank.checkIfAlive()) 
				{
					...
					// Fire a bullet
					if (Keyboard::isKeyPressed(Keyboard::Space) || Keyboard::isKeyPressed(Keyboard::RControl))
					{
				...
				if (chaserLife > 0)
				{
					if (!chaser.isInMotion())
					{
						...
						if (okayGo == 1)
						{
							chaser.moveChaser('r', 0.1);
							chaser.rotate();
						}
						else if (okayGo == 2)
						{
							chaser.moveChaser('l', 0.1);
							chaser.rotate();
						}
					}
				...
				if (opt == 4)
				{
					...
					if (opt == 7)
					{
						...
						while (mainScreen.pollEvent(menuEvent))
						{
							...
							string1 << "How to play" << endl << "the player can be controlled using the Keyboard"<<endl<<"." ;
